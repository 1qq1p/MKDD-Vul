...
contract CWVToken is StandardToken,owned {
    ...
    uint public time_on_trademarket; 

    uint public time_end_sale; 
    
    uint public angels_lock_days; 

    uint public team_total_lock_days; 

    uint public team_release_epoch; 
    ...
    mapping (address => uint) angels_locks;
    ...
    uint public epoch_release_count; 

    uint calc_unit = 1 days ;
    ...
    function CWVToken() public{
        ...
        epoch_release_count = team_lock_count/(team_total_lock_days/team_release_epoch);
    ...
    function unlockAngelAccounts(address[] _batchOfAddresses) public onlyOwner returns (bool success)   {
        ...
        require( now > time_on_trademarket + angels_lock_days );
